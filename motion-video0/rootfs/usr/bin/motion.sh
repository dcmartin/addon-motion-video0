    ...
    while true; do
      ...
      if [ "${config:-null}" != 'null' ]; then
        ...
        if [ -e /config/setup.json ]; then
          ...
          # camera_restart
          tf=$(addon::setup.update 'camera_restart' 'MOTION_CAMERA_RESTART') && update=$((update+tf))
          # camera_any
          tf=$(addon::setup.update 'camera_any' 'MOTION_CAMERA_ANY') && update=$((update+tf))
          # router
          ...
          # media
          tf=$(addon::setup.update 'media.save' 'MOTION_MEDIA_SAVE') && update=$((update+tf))
          tf=$(addon::setup.update 'media.mask' 'MOTION_MEDIA_MASK') && update=$((update+tf))
          # overview
          ...
          # yolo
          tf=$(addon::setup.update 'yolo.config' 'MOTION_YOLO_CONFIG') && update=$((update+tf))
          tf=$(addon::setup.update 'yolo.ip' 'MOTION_YOLO_IP') && update=$((update+tf))
          # USERS
          ...
          # detected.person
          tf=$(addon::setup.update 'person.entity' 'MOTION_DETECTED_PERSON_ENTITY') && update=$((update+tf))
          tf=$(addon::setup.update 'person.ago' 'MOTION_DETECTED_PERSON_AGO') && update=$((update+tf))
          tf=$(addon::setup.update 'person.deviation' 'MOTION_DETECTED_PERSON_DEVIATION') && update=$((update+tf))
          tf=$(addon::setup.update 'person.notify' 'MOTION_DETECTED_PERSON_NOTIFY') && update=$((update+tf))
          tf=$(addon::setup.update 'person.speak' 'MOTION_DETECTED_PERSON_SPEAK') && update=$((update+tf))
          tf=$(addon::setup.update 'person.tune' 'MOTION_DETECTED_PERSON_TUNE') && update=$((update+tf))
          # detected.vehicle
          tf=$(addon::setup.update 'vehicle.entity' 'MOTION_DETECTED_VEHICLE_ENTITY') && update=$((update+tf))
          tf=$(addon::setup.update 'vehicle.ago' 'MOTION_DETECTED_VEHICLE_AGO') && update=$((update+tf))
          tf=$(addon::setup.update 'vehicle.deviation' 'MOTION_DETECTED_VEHICLE_DEVIATION') && update=$((update+tf))
          tf=$(addon::setup.update 'vehicle.notify' 'MOTION_DETECTED_VEHICLE_NOTIFY') && update=$((update+tf))
          tf=$(addon::setup.update 'vehicle.speak' 'MOTION_DETECTED_VEHICLE_SPEAK') && update=$((update+tf))
          tf=$(addon::setup.update 'vehicle.tune' 'MOTION_DETECTED_VEHICLE_TUNE') && update=$((update+tf))
          # detected.animal
          tf=$(addon::setup.update 'animal.entity' 'MOTION_DETECTED_ANIMAL_ENTITY') && update=$((update+tf))
          tf=$(addon::setup.update 'animal.ago' 'MOTION_DETECTED_ANIMAL_AGO') && update=$((update+tf))
          tf=$(addon::setup.update 'animal.deviation' 'MOTION_DETECTED_ANIMAL_DEVIATION') && update=$((update+tf))
          tf=$(addon::setup.update 'animal.notify' 'MOTION_DETECTED_ANIMAL_NOTIFY') && update=$((update+tf))
          tf=$(addon::setup.update 'animal.speak' 'MOTION_DETECTED_ANIMAL_SPEAK') && update=$((update+tf))
          tf=$(addon::setup.update 'animal.tune' 'MOTION_DETECTED_ANIMAL_TUNE') && update=$((update+tf))
          # detected.entity
          tf=$(addon::setup.update 'entity.name' 'MOTION_DETECT_ENTITY') && update=$((update+tf))
          tf=$(addon::setup.update 'entity.ago' 'MOTION_DETECTED_ENTITY_AGO') && update=$((update+tf))
          tf=$(addon::setup.update 'entity.deviation' 'MOTION_DETECTED_ENTITY_DEVIATION') && update=$((update+tf))
          tf=$(addon::setup.update 'entity.notify' 'MOTION_DETECTED_ENTITY_NOTIFY') && update=$((update+tf))
          tf=$(addon::setup.update 'entity.speak' 'MOTION_DETECTED_ENTITY_SPEAK') && update=$((update+tf))
          tf=$(addon::setup.update 'entity.tune' 'MOTION_DETECTED_ENTITY_TUNE') && update=$((update+tf))
        fi
